...
awk "/^$chosen$/{p=1;print;next} p&&/$eps1/{p=0};p" "$tmpfile" | xclip -selection clipboard && notify-send -i "$HOME/coding/st/icons/terminal.png" "$TERMINAL" "Command output copied to clipboard"
